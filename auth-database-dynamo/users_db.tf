resource "aws_dynamodb_table" "user-table" {
  name           = "identity-server-users-table"
...
resource "aws_appautoscaling_target" "user_table_read_target" {
  max_capacity       = 20
  min_capacity       = 1
  resource_id        = "table/${aws_dynamodb_table.identity-server-users-table.name}"
  scalable_dimension = "dynamodb:table:ReadCapacityUnits"
  service_namespace  = "dynamodb"
}

resource "aws_appautoscaling_policy" "users_table_read_policy" {
  name               = "DynamoDBReadCapacityUtilization:${aws_appautoscaling_target.user_table_read_target.resource_id}"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.user_table_read_target.resource_id
  scalable_dimension = aws_appautoscaling_target.user_table_read_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.user_table_read_target.service_namespace
